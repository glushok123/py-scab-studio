...
def initRemovePostBorder(self):
    ...
    with futures.ThreadPoolExecutor(max_workers=self.count_cpu) as ex:
        ...
        for result in ex.map(self.removePostBorder, listOfFiles):
            print(str(result) + " (Удаление черной рамки ДОП)")
            #procent = procent + 1
            #self.statusLoaded(int((procent * 100) / countFile))

def removePostBorder(self, imageName):
    imageName = imageName.replace('\\', '/')
    imageName = imageName.replace(str(self.fileurl), '')
    array = imageName.split("/")
    urlDir = imageName.replace(array[-1], '')
    ...
    try:
        ...
        if points[1][1] < height * 0.8 or points[3][0] < width * 0.6:
            img = Image.open(self.fileurl + imageName)

            new_img = ImageOps.expand(img)
            new_img.save(self.directoryName + imageName, dpi=(self.dpi, self.dpi))
        else:
            warped = four_point_transform(image, points)
            nameFull = self.directoryName + imageName
